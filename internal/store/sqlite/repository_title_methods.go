...
func (r sonarrTitleRepo) NeedTMDBSync(ctx context.Context) ([]int64, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT DISTINCT tvdb_id FROM sonarr_title WHERE tvdb_id NOT IN (SELECT tvdb_id FROM tmdb_title) ORDER BY tvdb_id`)
	if err != nil {
...
func (r sonarrTitleRepo) WithTMDBTitles(ctx context.Context) ([]SonarrTitle, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT s.id,s.tvdb_id,s.sno,s.main_title,COALESCE(t.title,s.title),s.clean_title,s.season_number,s.monitored,s.valid_status,s.create_time,s.update_time,s.series_id FROM sonarr_title s LEFT JOIN tmdb_title t ON t.tvdb_id=s.tvdb_id AND t.valid_status=1 ORDER BY s.update_time DESC,s.id DESC`)
	if err != nil {
...
func (r tmdbTitleRepo) FindByTVDBID(ctx context.Context, id int64) ([]TMDBTitle, error) {
	page, err := r.Page(ctx, TMDBTitleFilter{Page: PageInput{Size: 200}, TVDBID: &id})
