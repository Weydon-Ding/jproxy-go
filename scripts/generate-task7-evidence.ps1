...
Invoke-Gate "task7_qa" "go" @("test","./internal/app","-run","Test(Task7|RootMux_rejectsPrimary|RootMux_rejectsMalformed|RootMux_rejectsOversized|RootMux_ruleCancellation)","-count=1","-v")
...
Invoke-Gate "full_test" "go" @("test","./...","-count=1","-shuffle=on"); Invoke-Gate "build" "go" @("build","./cmd/jproxy"); Invoke-Gate "vet" "go" @("vet","./..."); Invoke-Gate "lint" "golangci-lint" @("run"); Invoke-Gate "diff_check" "git" @("diff","--check"); Invoke-Gate "redaction_self_test" "pwsh" @("-NoProfile","-File","scripts/test-evidence-redaction.ps1","-Path",$output,"-SelfTest"); Invoke-Gate "redaction_scan" "pwsh" @("-NoProfile","-File","scripts/test-evidence-redaction.ps1","-Path",$output); Invoke-Gate "race" "go" @("test","-race","./...","-count=1","-shuffle=on") -Race
