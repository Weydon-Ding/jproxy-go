param([string]$OutputPath = ".omo/evidence/task-9-complete-jproxy-migration.txt")
...
[IO.File]::WriteAllText($output, "timestamp_utc=$([DateTime]::UtcNow.ToString('o'))`ncommit_under_test=$((git rev-parse HEAD).Trim())`nevidence_generator=scripts/generate-task9-evidence.ps1`n", $utf8)
Invoke-Gate "task9_qa" "go" @("test","./internal/app","-run","TestTask9_rootMuxMeasuresLiveTitleSyncContracts","-count=1","-v")
Invoke-Gate "focused_gate" "go" @("test","./internal/sync/title","./internal/runtime","./internal/api/title","./internal/store/sqlite","./internal/app","-count=10","-shuffle=on","-v")
